...
class MainScreen(QWidget):
    ...
    def load_images(self):
        """Charge 20 images aléatoires dans le grid_layout depuis le dossier img_align_celeba"""
        img_folder = "/Users/laura.forestier/Documents/4BIM_S2/Développement Logiciel/Projet-4BIM/img_align_celeba"
...
class PortraitWorkspace(QWidget):
    ...
    def toggle_selection(self, icon, btn):
        if icon in self.selected_portraits:
            ...
            # Supprimer l'image et la remettre à la fin de la liste
            if icon in self.selected_images:
                self.selected_images.remove(icon)
                self.selected_images.append(icon)
        else:
            self.selected_portraits[icon] = btn
            btn.setStyleSheet("border: 2px solid red; background-color: black;")

            # Ajouter l'image à la fin si elle n'est pas déjà présente
            if icon not in self.selected_images:
                self.selected_images.append(icon)
